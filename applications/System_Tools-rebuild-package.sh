...
rpmbuild-error() {
    if [ "$?" != "0" ]; then
        ...
        kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-rebuild-rpm.svgz --caption="Rebuild RPM Package" \
...
rpmmacros-no-cflags() {
    ...
    kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-rebuild-rpm.svgz --caption="Rebuild RPM Package | B.O.O.=\"$BINOPT\"" \
    ...
    if [ "$?" != "0" ]; then
        BINOPT=$(kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-rebuild-rpm.svgz --caption="Rebuild RPM Package" --combobox="Choose Binary Optimization Option(s)" \
...
install-package() {
    ...
    if [ ! -s package.tmp ]; then
        ...
        play /tmp/speak.wav 2> /dev/null
        rm -fr /tmp/speak*
        kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-rebuild-rpm.svgz --caption="Rebuild RPM Package" --error="No packages for install. Try Again." 2> /dev/null
        exit 0
    fi
    ...
    INSTALLPKG=$(kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-rebuild-rpm.svgz --caption="Rebuild RPM Package" --separate-output \
...
sudo-no-timeout
setterm -cursor off
PRIORITY="$(kdialog --geometry 100x100 --icon=/usr/share/icons/hicolor/scalable/apps/ks-rebuild-rpm.svgz --caption="Rebuild RPM Package" \
...
PACKAGESOURCE=$(kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-rebuild-rpm.svgz --caption="Rebuild RPM Package" --yesnocancel "Have the source RPM package?" 2> /dev/null)
EXIT=$?
if-cancel-exit
...
if [ "$EXIT" = "1" ]; then
    ...
    APPNAME=$(kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-rebuild-rpm.svgz --caption="Rebuild RPM Package" --separate-output \
    ...
    if [ "$APPNAME" = "Other" ]; then
        APPNAME=$(kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-rebuild-rpm.svgz --caption="Rebuild RPM Package" \
...
if [ "$EXIT" = "0" ]; then
    SOURCEFILE=$(kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-rebuild-rpm.svgz --caption="Rebuild RPM Package - Source RPM File" --multiple \
