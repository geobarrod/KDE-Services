...
STATE=$(kdialog --icon=connect-sentry-on --caption="Connect Sentry" --combobox="Choose Status" Enabled Disabled --default Enabled 2> /dev/null)
if-cancel-exit

if [ "$STATE" = "Enabled" ]; then
    if [ "$(cat $PID 2> /dev/null)" = "$CHECKPID" ] && [ -s $PID ]; then
        kdialog --icon=connect-sentry-on --caption="Connect Sentry" --sorry="Already Running." 2> /dev/null
        exit 0
    fi
    ...
    PORTS=$(kdialog --icon=connect-sentry-on --caption="Connect Sentry" \
    ...
    if [ "$?" != "0" ]; then
        kdialog --icon=application-exit --title="Connect Sentry - Aborted By Wrong Syntax" --passivepopup="$PORTS" 2> /dev/null
        exit 0
    fi
    
    echo $$ > $PID
    kdialog --icon=connect-sentry-on --title="Connect Sentry" --passivepopup="[Enabled]" 2> /dev/null
    ...
    while true; do
        ...
        if [ "$EXIT" != "0" ]; then
            kdialog --icon=connect-sentry-established --title="Connection Established" --passivepopup="$EST" 2> /dev/null
        fi
    done
    ...
    rm -f $PID 2> /dev/null
    kdialog --icon=connect-sentry-off --title="Connect Sentry" --passivepopup="[Disabled]" 2> /dev/null

elif [ "$STATE" = "" ]; then
    kdialog --icon=connect-sentry-on --caption="Connect Sentry" \
