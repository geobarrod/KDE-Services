...
if [ "$STATE" = "Enabled" ]; then
    ...
    if [ "$?" != "0" ]; then
        kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Connect Sentry - Aborted By Wrong Syntax" --passivepopup="$PORTS" 2> /dev/null
        exit 0
    fi
    
    echo $$ > $PID
    kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-sentry-on.svgz --title="Connect Sentry" --passivepopup="[Enabled]" 2> /dev/null
    ...
    while true; do
        ...
        if [ "$EXIT" != "0" ]; then
            kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-sentry-warning.svgz --title="Connection Established - $(date "+%b %d %H:%M %Y")" --passivepopup="$EST" 2> /dev/null
        fi
    done
