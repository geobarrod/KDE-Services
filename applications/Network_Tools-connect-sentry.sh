...
STATE=$(kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-sentry-on.png --caption="Connect Sentry" --combobox="Choose Status" Enabled Disabled --default Enabled 2> /dev/null)
if-cancel-exit

if [ "$STATE" = "Enabled" ]; then
    if [ "$(cat $PID 2> /dev/null)" = "$CHECKPID" ] && [ -s $PID ]; then
        kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-sentry-on.png --caption="Connect Sentry" --sorry="Already Running." 2> /dev/null
        exit 0
    fi
    ...
    PORTS=$(kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-sentry-on.png --caption="Connect Sentry" \
    ...
    if [ "$?" != "0" ]; then
        kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-error.png --title="Connect Sentry - Aborted By Wrong Syntax" --passivepopup="$PORTS" 2> /dev/null
        exit 0
    fi
    
    echo $$ > $PID
    kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-sentry-on.png --title="Connect Sentry" --passivepopup="[Enabled]" 2> /dev/null
    ...
    rm -f $PID 2> /dev/null
    kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-sentry-off.png --title="Connect Sentry" --passivepopup="[Disabled]" 2> /dev/null

elif [ "$STATE" = "" ]; then
    kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-sentry-on.png --caption="Connect Sentry" \
