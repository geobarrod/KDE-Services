...
check_stderr() {
    if [ "$?" != "0" ]; then
        kdialog --icon=application-exit --title="Samba Shares Mounter" \
...
mount_share() {
    ...
    if [ "$VIEW_SHARE" = "" ]; then
        kdialog --icon=application-x-smb-server --caption="Samba Shares Mounter" --sorry="$HOST not have samba share directory." 2> /dev/null &
        exit 0
    fi
    
    SHARE=$(kdialog --icon=application-x-smb-server --caption="Samba Shares Mounter" \
    ...
    for share in $(mount|grep -w cifs|awk -F " " '{print $3}'); do
        if [ "$share" = "$HOME/SaMBa-Shares/$HOST-$SHARE" ]; then
            kdialog --icon=application-x-smb-server --caption="Samba Shares Mounter" \
    ...
    if [ "$?" = "0" ]; then
        USERNAME=$(kdialog --icon=application-x-smb-server --caption="Samba Shares Mounter" --inputbox="Enter Samba Username" 2> /dev/null)
        if-cancel-exit
        PASSWD=$(kdialog --icon=application-x-smb-server --caption="Samba Shares Mounter" \
        ...
        check_stderr
        kdialog --icon=application-x-smb-server --title="Samba Shares Mounter" \
...
OPTION=$(kdialog --icon=application-x-smb-server --caption="Samba Shares Mounter" \
       --combobox="Select Option" "Mount Samba Share Directory" "Unmount Samba Share Directory" --default "Mount Samba Share Directory" \
       2> /dev/null)
if-cancel-exit
    
if [ "$OPTION" = "" ]; then
    kdialog --icon=application-exit --title="Samba Shares Mounter" --passivepopup="[Error] Please select one option. Try again." 2> /dev/null
    exit 0
elif [ "$OPTION" = "Mount Samba Share Directory" ]; then
    SERVER=$(cat ~/.kde-services/machines)
    HOST=$(kdialog --icon=application-x-smb-server --caption="Samba Shares Mounter" --combobox="Select Hostname or IP Address" $SERVER \
         --default $(head -n1 ~/.kde-services/machines) 2> /dev/null)
    
    if [ "$?" != "0" ]; then
        HOST=$(kdialog --icon=application-x-smb-server --caption="Samba Shares Mounter" \
    ...
    if [ "$VIEW_SHARE" = "" ]; then
        kdialog --icon=application-x-smb-server --caption="Samba Shares Mounter" --sorry="Not have mounted samba share directory." 2> /dev/null &
    ...
    kdialog --icon=application-x-smb-server --title="Samba Shares Mounter" --passivepopup="[Finished] $SHARE Unmounted." 2> /dev/null
    VIEW_SHARE=$(mount|grep -w cifs|awk -F " " '{print $3}')
