...
check_stderr() {
    if [ "$?" != "0" ]; then
        kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Samba Shares Mounter" \
...
if [ "$OPTION" = "" ]; then
    kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-error.svgz --title="Samba Shares Mounter" --passivepopup="[Error] Please select one option. Try again." 2> /dev/null
    exit 0
    ...
    kdialog --icon=/usr/share/icons/hicolor/scalable/apps/ks-smbfs.svgz --title="Samba Shares Mounter" --passivepopup="[Finished] $SHARE Unmounted." 2> /dev/null
    VIEW_SHARE=$(mount|grep -w cifs|awk -F " " '{print $3}')
