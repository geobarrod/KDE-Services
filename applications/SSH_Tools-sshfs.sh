...
OPTION=$(kdialog --icon=sshfs --caption="SSH Tools - Mountpoint To Remote Directory" \
...
if [ "$OPTION" = "Mount Remote Directory" ]; then
    ...
    LOGIN=$(kdialog --icon=sshfs --caption="SSH Tools - Mountpoint To Remote Directory" --combobox="Select User" $USER root \
          --default $USER 2> /dev/null)
    if-cancel-exit
    HOST=$(kdialog --icon=sshfs --caption="SSH Tools - Mountpoint To Remote Directory" --combobox="Select Hostname or IP Address" $SERVER  \
           --default $(head -n1 ~/.kde-services/machines) 2> /dev/null)
    
    if [ "$?" -gt "0" ]; then
        HOST=$(kdialog --icon=sshfs --caption="SSH Tools - Mountpoint To Remote Directory" \
    ...
    if [ "$MOUNTPOINT" = "" ]; then
        kdialog --icon=sshfs --caption="SSH Tools - Mountpoint To Remote Directory" \
    ...
    kdialog --icon=sshfs --title="SSH Tools - Mountpoint To Remote Directory" --passivepopup="[Finished]" 2> /dev/null
fi
