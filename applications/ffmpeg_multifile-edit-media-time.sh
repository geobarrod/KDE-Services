...
if-ffmpeg-cancel() {
    if [ "$?" != "0" ]; then
        kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-error.png --title="Editing time of ${i##*/} from $STIME to $ETIME" \
...
progressbar-start() {
    ...
    DBUSREF=$(kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-media-edit-time.png --caption="Edit Time from Media Files" --progressbar "				" $COUNTFILES)
}
...
elapsedtime() {
    if [ "$ELAPSED_TIME" -lt "60" ]; then
        kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-media-edit-time.png --title="Edit Time from Media Files" \
...
PRIORITY="$(kdialog --geometry 100x100 --icon=/usr/share/icons/hicolor/512x512/apps/ks-media-edit-time.png --caption="Edit Time from Media Files" \
...
FILES=$(kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-media-edit-time.png --caption="Edit Time from Media Files" --multiple --getopenfilename "$DIR" "*.3GP *.3gp *.AVI *.avi *.DAT \
...
DESTINATION=$(kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-media-edit-time.png --caption="Destination Video Files" --getexistingdirectory "$DIR" 2> /dev/null)
if-cancel-exit

STIME=$(kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-media-edit-time.png --caption="Edit Time from Media Files" \
                --inputbox="Enter start cut time in hh:mm:ss format" 00:00:00)
if-cancel-exit

ETIME=$(kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-media-edit-time.png --caption="Edit Time from Media Files" \
