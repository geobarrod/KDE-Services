...
progressbar-start() {
	kdialog --icon=ks-dropbox --title="Dropbox Tools" --print-winid --progressbar "$(date) - Processing..." /ProcessDialog|grep -o '[[:digit:]]*' > $PB_PIDFILE
}
...
EOF
	...
	if [ "$KDE_SESSION_VERSION" == "5" ]; then
		qdbus-qt5 org.kde.klipper /klipper org.kde.klipper.klipper.setClipboardContents "$($DROPBOX puburl $DROPBOX_PATH/Public/index.html)"
	elif [ "$KDE_SESSION_VERSION" == "6" ]; then
		qdbus6 org.kde.klipper /klipper org.kde.klipper.klipper.setClipboardContents "$($DROPBOX puburl $DROPBOX_PATH/Public/index.html)"
	fi
}
