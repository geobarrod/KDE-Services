...
check_stderr() {
    if [ "$EXIT" != "0" ]; then
	qdbus $DBUSREF close
	kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-error.png \
                       --title="Dropbox Tools" \
...
clipboard_url() {
    LOGIN=$(kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-dropbox.png --caption="Dropbox Tools" \
...
copy() {
    if [ -f $HOME/.dropbox/host.db ]; then
        DBUSREF=$(kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-dropbox.png --caption="Dropbox Tools" --progressbar " " /ProgressDialog)
...
Terminal=false
Type=Application
Icon=/usr/share/icons/hicolor/512x512/apps/ks-dropbox.png
