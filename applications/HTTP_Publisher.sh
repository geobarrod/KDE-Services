...
if [ "$UID" != "0" ]; then
    PORT=$(kdialog --icon=folder-remote --caption="HTTP Publisher" \
...
if [ "$PORT" -lt "1" ] || [ "$PORT" -gt "65535" ]; then
    if [ "$UID" != "0" ]; then
        kdialog --icon=application-exit --title="HTTP Publisher" --passivepopup="[Error]   Port number $PORT out of range. \
...
if [ "$EXIT" != "0" ]; then
    ...
    if [ "$EXIT" != "0" ]; then
        kdialog --icon=application-exit --title="HTTP Publisher" \
    ...
    if [ "$PORT" != "80" ]; then
        kdialog --icon=folder-remote --caption="HTTP Publisher" --yes-label Background --no-label Stop \
    ...
    kdialog --icon=application-exit --title="HTTP Publisher" --passivepopup="[Error]   Port number $PORT already in use, change it and try again."
fi
