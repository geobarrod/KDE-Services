...
if [ "$UID" != "0" ]; then
    PORT=$(kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-folder-public-web.png --caption="HTTP Server" \
...
if [ "$PORT" -lt "1" ] || [ "$PORT" -gt "65535" ]; then
    if [ "$UID" != "0" ]; then
        kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-error.png --title="HTTP Server" --passivepopup="[Error]   Port number $PORT out of range. \
...
if [ "$EXIT" != "0" ]; then
    ...
    if [ "$EXIT" != "0" ]; then
        kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-error.png --title="HTTP Server" \
    ...
    if [ "$PORT" != "80" ]; then
        kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-folder-public-web.png --caption="HTTP Server" --yes-label Background --no-label Stop \
    ...
    kdialog --icon=/usr/share/icons/hicolor/512x512/apps/ks-error.png --title="HTTP Server" --passivepopup="[Error]   Port number $PORT already in use, change it and try again."
fi
