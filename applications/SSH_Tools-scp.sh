...
if [ -s ~/.kde-services/machines ]; then
    ...
    HOST=$(kdialog --icon=utilities-terminal --caption="SSH Tools - Send To" --combobox="Select Hostname or IP Address" $SERVER \
           --default $(head -n1 ~/.kde-services/machines) 2> /dev/null)
    if-cancel-exit
    LOGIN=$(kdialog --icon=utilities-terminal --caption="SSH Tools - Send To $HOST" --combobox="Select User" $USER root \
    ...
    rm -fr /tmp/speak*
    kdialog --icon=utilities-terminal --title="SSH Tools - Send $(basename $1) To $HOST by $LOGIN" --passivepopup="[Finished]" 2> /dev/null
    exit 0
else
